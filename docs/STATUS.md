...
*   **Room Database:** Đã khởi tạo `AppDatabase` cùng các thực thể chuẩn theo yêu cầu: `UserEntity`, `LibraryItemEntity`, `ChapterProgressEntity` và các Dao tương ứng.
*   **Local Preferences:** Cài đặt `UserPreferencesDataStore` lưu trữ các cài đặt như `reader_quality`, `language`, `theme_mode`.
...
*   **Detail Flow & Reader Flow:**
    *   **MangaDetailScreen:** Hiển thị thông tin truyện, bìa ảnh, tóm tắt và danh sách chapter nhóm theo Volume (Quyển). Có kết hợp trạng thái lưu trữ ở Room để hiện `UNREAD`, `READING`, `COMPLETED` cho từng chapter.
    *   **ReaderScreen:** Trình đọc truyện cuộn dọc mượt mà. Hỗ trợ chạm để ẩn/hiện thanh công cụ (Overlay), theo dõi tiến độ trang hiện tại (`lastReadPage`), tự động lưu lại tiến độ khi thoát.
...
### Known Issue
* Thanh điều hướng hiển thị chưa đẹp
